...
def main(cfg: DictConfig):
    ...
    # Count parameters
    num_params = sum(
        param.size for param in jax.tree_util.tree_leaves(nnx.state(model, nnx.Param))
    )
    ...
    save_summary_steps = resolve_default(cfg, 'train.save_summary_steps', 50)
    ...
    for epoch in epoch_iterator:
        ...
        train_metrics = train_epoch(
            model, optimizer, train_inputs,
            batch_size=batch_size,
            reconstruction_weight=reconstruction_weight,
            prediction_weight=prediction_weight
        )

        # Validate (and potentially checkpoint) every save_summary_steps
        should_checkpoint = (epoch + 1) % save_summary_steps == 0 or (epoch + 1) == num_epochs
        ...
        if should_checkpoint:
            ...
            # For non-checkpoint epochs, just use last validation metrics
            # (or skip validation entirely to save time)
            val_metrics = {
                'loss': best_val_loss,
