...
def save(filename, dic, compression="gzip", compression_opts=5, chunked_datasets=None):
    ...
    compression_opts : int, optional
        Compression level (0-9 for gzip). Default is 5x
...
def recursively_save_dict_contents_to_group(
    h5file, path, dic, compression="gzip", compression_opts=5, chunked_datasets=None
):
    """
    ...
    for key, item in iterator:
        ...
        if isinstance(
            item,
            (jnp.ndarray, np.ndarray, np.int64, np.float64, int, float, str, bytes),
        ):
...
def recursively_convert_appropriate_dicts_to_lists(data):
    ...
    if isinstance(data, dict):
        ...
        processed_data = {
            k: recursively_convert_appropriate_dicts_to_lists(v)
...
def load(filename, ASLIST=False, enable_jax=False, auto_convert_lists=True):
    ...
    with h5py.File(filename, "r") as h5file:
        out = recursively_load_dict_contents_from_group(
            h5file, "/", enable_jax=enable_jax
        )
...
def save_reference_clips_chunked(
    filename,
    reference_clips_dict,
    compression="gzip",
    compression_opts=3,
    clips_per_chunk=1,
):
    ...
    if "qpos" in reference_clips_dict:
        _qpos_shape = reference_clips_dict["qpos"].shape
    ...
    print("Saving ReferenceClips with chunking strategy:")
    ...
    for key, item in h5file[path].items():
        if isinstance(item, h5py.Dataset):
            ...
            if (
                ...
                and data.dtype.kind not in ["S", "U"]
            ):
            ...
            ans[key] = recursively_load_dict_contents_from_group(
                h5file, path + key + "/", enable_jax=enable_jax
            )
    return ans
