...
class TiDHy(nn.Module):
    def __init__(self, params, device, show_progress=True, show_inf_progress=False):
        ...
        if params.loss_type == "BCE":
            self.spatial_decoder = nn.Sequential(
                nn.Linear(params.r_dim, params.input_dim, bias=True), nn.Sigmoid()
            )
        ...
        if params.nonlin_decoder:
            self.spatial_decoder = nn.Sequential(
                nn.Linear(params.r_dim, params.hyper_hid_dim, bias=True),
            ...
            self.spatial_decoder = nn.Sequential(
                nn.Linear(params.r_dim, params.input_dim, bias=True),
            )
        #### Initialize Weights #####
        ...
        if params.low_rank_temp:
            self.temporal = nn.Parameter(
                torch.randn((params.mix_dim, params.r_dim, 2), requires_grad=True)
            )
        else:
            self.temporal = nn.Parameter(
                torch.randn(
                    params.mix_dim, params.r_dim * params.r_dim, requires_grad=True
                )
            )
        nn.init.orthogonal_(self.temporal)
        if params.dyn_bias:
            self.temporal_bias = nn.Parameter(
                torch.randn(1, params.r_dim, requires_grad=True)
            )
        ...
        self.hypernet = nn.Sequential(
            ...
            (
                nn.Linear(
                    params.hyper_hid_dim, params.mix_dim + params.r_dim, bias=False
                )
                if params.dyn_bias
                else nn.Linear(params.hyper_hid_dim, params.mix_dim, bias=True)
            ),  # weight and +1/2x for bias
            nn.ReLU(),
        )
        ...
        self.tol = params.tol
        self.r2_state = None
        self.r_state = None  # Initialize r_state attribute
        ...
        self.log_msg = ""  # Initialize log_msg attribute
        self.l0_inf = None  # Initialize l0_inf attribute
    ...
    def forward(self, X):
        ...
        spatial_loss_rhat = (
            self.spat_loss(self.spatial_decoder(r), X[:, 0])
            .view(batch_size, -1)
            .sum(1)
            .mean(0)
        )
        ...
        for t in t_range:
            ...
            r, r2, _r2_loss = self.inf(X[:, t], r_p, r2.detach().clone())
            # learning
            ...
            if self.use_r2_decoder:
                ...
                r2_losses += (
                    torch.pow(r2 - r2_hat, 2).view(batch_size, -1).sum(1).mean(0)
                )
            spatial_loss_rhat += (
                self.spat_loss(x_hat, X[:, t]).view(batch_size, -1).sum(1).mean(0)
            )  ##### Spatial Loss due to x_hat #####
            spatial_loss_rbar += (
                self.spat_loss(x_bar, X[:, t]).view(batch_size, -1).sum(1).mean(0)
            )  ###### Spatial Loss due to x_bar #####
    ...
    def inf(self, x, r_p, r2):
        ...
        milestones_r = [
            block
            for _, block in enumerate(
                range(self.max_iter // 10, self.max_iter, self.max_iter // 10)
            )
        ]
        _milestones_r2 = [
            block
            for _, block in enumerate(
                range(self.max_iter // 5, self.max_iter, self.max_iter // 10)
            )
        ]
        scheduler = [MultiStepLR(optim_r, milestones=milestones_r, gamma=0.5)]
        converged = False
        i = 0
        r2_loss = []
        ...
        while (converged == False) & (i < self.max_iter):
            ...
            r_bar, _V_t, w = self.temporal_prediction(r_p, r2)
            # prediction error
            if self.use_r2_decoder:
            ...
            if self.grad_norm_inf:
                ...
                weights, losses, optimizers, r, r2 = self.grad_norm_inf_step(
                    r, r2, losses, optimizers, i
                )
            else:
            ...
            with torch.no_grad():
                if self.batch_converge:
                    r2_converge = torch.linalg.norm(r2 - old_r2) / (
                        torch.linalg.norm(old_r2) + 1e-16
                    )
                    r_converge = torch.linalg.norm(r - old_r) / (
                        torch.linalg.norm(old_r) + 1e-16
                    )
                else:
                    r2_converge = torch.linalg.norm(r2 - old_r2, dim=-1) / (
                        torch.linalg.norm(old_r2, dim=-1) + 1e-16
                    )
                    r_converge = torch.linalg.norm(r - old_r, dim=-1) / (
                        torch.linalg.norm(old_r, dim=-1) + 1e-16
                    )
            converged = torch.all(r_converge < self.tol) and torch.all(
                r2_converge < self.tol
            )
            ##### Show Inference Progress #####
            if self.show_inf_progress:
                self.log_msg = "inf_it:{}, r2_conv:{:.03f}, r_conv:{:.03f}, spat_loss:{:.02f}, temp_loss:{:.02f} ".format(
                    i,
                    torch.sum(r2_converge > self.tol),
                    torch.sum(r_converge > self.tol),
                    spatial_loss.item(),
                    temporal_loss.item(),
                )
                if self.grad_norm_inf:
                    self.log_msg += "gnorm_w:{}, ".format(
                        list(weights.detach().cpu().numpy())
                    )
        ...
        self.converge_warning_(
            i,
            "r/r2 did not converge: r2_conv:{}, r_conv:{}, ".format(
                torch.sum(r2_converge > self.tol).item(),
                torch.sum(r_converge > self.tol).item(),
            ),
        )
    ...
    def temporal_prediction(self, r, r2):
        ...
        if self.low_rank_temp:
            Vk = torch.bmm(self.temporal, self.temporal.permute(0, 2, 1)).reshape(
                self.mix_dim, -1
            )
            V_t = torch.matmul(w, Vk).reshape(batch_size, self.r_dim, self.r_dim)
        else:
            V_t = torch.matmul(w, self.temporal).reshape(
                batch_size, self.r_dim, self.r_dim
            )

        ###### Handle Bias if set #####
        if self.dyn_bias:
            r_hat = (
                torch.bmm(V_t, r.unsqueeze(2)) + (b * self.temporal_bias).unsqueeze(2)
            ).squeeze(
                dim=-1
            )  # (batch_size, r_dim)
        else:
            r_hat = (torch.bmm(V_t, r.unsqueeze(2))).squeeze(
                dim=-1
            )  # (batch_size, r_dim)
        return r_hat, V_t, w
    ...
    def init_code_(self, batch_size):
        r = torch.zeros(
            (batch_size, self.r_dim), requires_grad=True, device=self.device
        )
        r2 = torch.zeros(
            (batch_size, self.r2_dim), requires_grad=True, device=self.device
        )
        return r, r2
    ...
    def normalize(self):
        with torch.no_grad():
            if self.normalize_spatial:
                self.spatial_decoder[0].weight.data = F.normalize(
                    self.spatial_decoder[0].weight.data, dim=0
                )
    ...
    def grad_norm_inf_step(self, r, r2, loss, optimizer, iters):
        ...
        for i in range(len(loss)):
            if i == 0:
                dl_r, dl_r2 = torch.autograd.grad(
                    weights[i] * loss[i],
                    model_latents[i],
                    retain_graph=True,
                    create_graph=True,
    ...
    def evaluate_record(self, data_batch):
        ...
        )  # Input prediction from Dynamics
        I_hat = torch.zeros(
            (batch_size, T, input_dim)
        )  # Input prediction from Inference
        I = torch.zeros((batch_size, T, input_dim))  # True input
        R_bar = torch.zeros(
            (batch_size, T, self.r_dim)
        )  # Latent prediction from dynamics
        R_hat = torch.zeros(
            (batch_size, T, self.r_dim)
        )  # Latent prediction from Inference
        R2_hat = torch.zeros(
            (batch_size, T, self.r2_dim)
        )  # Higher order latent prediction from Inference
        ...
        spatial_loss_rhat = torch.zeros(
            (batch_size, T, self.input_dim)
        )  # Reconstruction Loss
        spatial_loss_rbar = torch.zeros(
            (batch_size, T, self.input_dim)
        )  # Reconstruction Loss
        if self.dyn_bias:
            b = torch.zeros((batch_size, T, self.r_dim))  # Bias
        Ut = torch.zeros(
            (batch_size, T, self.r_dim, self.r_dim)
        )  # Temporal prediction matrices
        # initialize embedding
        r, r2 = self.init_code_(batch_size)
        R_bar[:, 0] = r.detach().clone().cpu()
        R2_hat[:, 0] = r2.detach().clone().cpu()
        I_bar[:, 0] = self.spatial_decoder(r).detach().clone().cpu()
        spatial_loss_rbar[:, 0] = self.spat_loss(
            self.spatial_decoder(r), data_batch[:, 0]
        )
        # p(r_1 | I_1)
        ...
        I[:, 0] = data_batch[:, 0]
        _r_first = r.detach().clone()
        spatial_loss_rhat[:, 0] = self.spat_loss(
            self.spatial_decoder(r), data_batch[:, 0]
        )

        for t in tqdm(range(1, T), leave=False):
            _r2_p = r2.detach().clone()
            r_p = r.detach().clone()

            # hypernet prediction
            r_bar, V_t, _w = self.temporal_prediction(r_p, r2)
            ...
            wb = self.hypernet(r2)
            W[:, t] = (
                wb[:, : self.mix_dim].reshape(batch_size, -1).detach().clone().cpu()
            )
            if self.dyn_bias:
                b[:, t] = (
                    wb[:, self.mix_dim :].reshape(batch_size, -1).detach().clone().cpu()
        ...
        return (
            spatial_loss_rhat_avg,
            spatial_loss_rbar_avg,
            self.temp_weight * temp_loss.reshape(batch_size, -1).sum(1).mean(0),
            result_dict,
        )
